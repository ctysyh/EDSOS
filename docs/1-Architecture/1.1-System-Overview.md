...
3. **Loading-Time Centrality**  
   The loading phase is elevated to equal importance with compilation and runtime. EDSOS 不开放传统的 syscall 接口，而是定义了全面的桩函数，在加载期自动内联为最终的机器指令。被所加载代码描述的 AS 结构也在加载期被构建起骨架。

4. **Unified Control and Data Flow**  
   Control flow (via structured jumps) and data flow (via field binding) are co-designed within the AS model. 控制逻辑对应着变量字段，变量字段同样对应着控制逻辑，它们被统一在节点当中。
...
- 当没有专用 DPU 时，EDSOS 仍可按照单机模式正常工作，并允许被添加相关服务以运行在传统集群网络上。
