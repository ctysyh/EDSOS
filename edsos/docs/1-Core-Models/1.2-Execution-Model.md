...
- [1.2 Execution Model](#12-execution-model)
  ...
  - [1.2.2 Meta-AS：系统的全局结构化视图](#122-meta-as系统的全局结构化视图)
    - [1.2.2.1 系统服务组件与驱动程序作为 AS 实例](#1221-系统服务组件与驱动程序作为-as-实例)
...
节点基本结构由 AS 形式化模型规定，并由 Arxil 语言精确描述，详见[Arxil语法规范中的节点语法](../../../Arxil/docs/Arxil%20Language%20Specification.md#32-node-declaration)。其中，节点可以仅包含数据段（无指令段），称为退化节点，专门用于数据缓存与共享；包含有效指令段的普通节点可被调度执行，行为类似于协程。
...
这些操作本身内置于 Arxil 的语言关键字并各自对应EDSOS中的stub function。详见[第一类系统调用](../2-Architecture/2.5-Syscall-Model.md#251-第一类系统调用架构)。
...
### 1.2.2.1 系统服务组件与驱动程序作为 AS 实例

EDSOS 中系统服务组件（如文件、网络）和驱动程序同样以 AS 实例形式存在，通过结构共享与信号同步协作，并在结构上保持对等。
...
它们之间的差异仅体现在 Capability 授权范围与硬件访问权限上，而非执行模型本身。每个 AS 实例可包含一个或（分布运行在多个 CPU 核心时）多个 **SPN（System Proxy Node）** 作为与其他 AS 实例通信的代理，详见[第二类系统调用](../2-Architecture/2.5-Syscall-Model.md#252-第二类系统调用架构)。该设计在保留微内核安全边界的同时，通过结构化共享避免了传统微内核中因频繁 IPC 导致的性能瓶颈。
...
该设计大幅减小了攻击面：系统调用要么由调度器在本地处理（通过在[内联桩函数](../2-Architecture/2.3-Execution-Subsystem.md#23221-cooperative-scheduling-stub-driven)的封闭上下文中的特权切换），要么通过异步委托交由服务 AS 执行（完全隔离于当前 AS 的执行上下文）。

硅中介层自身可以类比于并称为实际的*内核*。这具备显著的*微内核*风格。值得注意的是，虽然硅中介层假设自身运行于裸金属之上，但是只要有正确的硬件指令接口实现，其完全可以作为虚拟机运行。
