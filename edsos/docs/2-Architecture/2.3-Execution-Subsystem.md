...
- **Scheduler-Driven Execution**: The per-core scheduler is the sole authority for node lifecycle transitions (`push`, `pop`, `lift`, `merge`) and migration decisions.
...
Each AS instance is assigned a globally unique **64-bit ASID** (Arbor Strux Identifier), stored in its root node. ASIDs are allocated by a dedicated system service (`/hw/resmgr`) and recorded in a distributed registry. Human-readable aliases may be bound to ASIDs (e.g., `/user/chrome_instance_01`).
...
- **Scope Elevation (optional)**: A executes `lift(CARN_X)` to promote it into an ancestor, enabling self-access;
- **Cross-AS Binding**:
  - Node B performs `lookup("CARN_X")` to obtain its virtual address;
  - B executes `push(CARN_X_VA, 'as_ctrn')` followed by `lift(CARN_X)`, integrating the CARN into its own ancestry;
...
3. **Declarative Configuration**: Reads system configuration from a designated path (e.g., `/etc/edsos/configuration.nix`), determining roles, services, security policies, and cluster membership;
...
1. **Global Resource Aggregation**: A dedicated System Resource Management Service collects local resource trees from all PMs via the DPU network and merges them into a **unified global hardware view**;
...
- **AS Structural Mutation**: Executing atomic operations such as `push`, `pop`, `lift`, `merge`, and `detach`, ensuring both intermediate and final AS topology states remain well-defined and causally consistent.
...
- **Trigger Timing**: Explicitly callable at any point within a node’s execution flow (e.g., via `edsos_wait()`, `edsos_push()`, or `edsos_lift()`).
...
- Receiving structural mutation notifications (e.g., `lift` or `merge`) that affect the ancestor chain.
