...
> TODO List
> 1. 明确调度器的含义：它作为硅中介层的主要成分，是一些不以节点形式与结构组织的*内存*（对应于经典的内核空间）、和“散落的”即以 Inline Stub 形式存在于各个节点实例的指令序列中且受到可信验证机制保护的*指令*。
> 2. 进一步明确 Per-Core 节点调度器实例持有的就绪链表：在实现上，它是一个用数组实现的链表，调度器每次直接从链表头取；但是链表本身可以被重排序，包括以 SDSC 形式包装的排序逻辑、乃至专门的调度优化 AS 节点实例。由此，将调度逻辑从热路径中分离出去。
> 3. 在此基础上，展开分析利用 SDSC 机制将上层逻辑拆分解耦、模块化，包括不限于负载均衡等。

---

> Notes
> 让我们首先深入梳理调度器（为名号的硅中介层主体）需要哪些数据。
> 它首先需要一个可靠的就绪链表和等待图。在这里，我们完全可以避免指针解引用开销。为此，进一步展开就绪链表的“数组实现的链表”。它整体是作为一个连续数组实现的。数组的每一项是一个结构体，类似于
> ```c
> struct ready_list_entry {
>     uint32_t prev_idx;
>     uint32_t next_idx;
>     struct node_info ready_node;
> }
> ```
> 整个数组类似于
> ```c
> ready_list_entry ready_list[MAX_ENTRY_COUNT];
> ```
> 类似的，服务于EDSOS信号量的等待图也同理。等待图主要包含两类数据，一类是每个现存的`semaphore_name → semaphore_id`及附属元信息，另一类是属于各个信号量的每个`blocked_node`。整个Per-Core数据结构逻辑上类似于一个二维数组，每一行对应了每一个semaphore条目，该行的每一列即当前semaphore条目的每一个阻塞节点；更精细的，每一行的列数是解耦的，但是都按相同的固定步长扩缩容，整个数据结构的行数同样是可以动态扩缩容的。为此，可以设计一个解耦的数据结构，类似于
> ```c
...
>     struct semaphore_entry semaphore[MAX_SEMAFPORE_COUNT];
>     struct blocked_list_segment segment[MAX_SEGMENT_COUNT];
> }
> ```
> 这里还可以进一步添加一些辅助的字段以快速定位可用的`blocked_list_segment`。
>
> 在系统初始化时，引导组件为CPU的每个核心选定一块按策略静态配置大小的物理内存区域，并作如上使用。这部分物理内存直接且仅供调度器访问，不会参与后续各AS实例的虚拟内存空间映射。
>
> ---
>
> 接着，考虑调度器的另一职责：准备节点实例的执行环境。这意味着调度器需要掌握节点实例所属的AS实例的页表基址（物理地址），以及节点实例在其虚拟内存空间的起始地址（虚拟地址）。
> 此外，考虑调度器在执行抢占式调度时，当前执行的节点实例会有大量寄存器状态且未自主保存进其自身中。为此，调度器需要准备一块大小恰当的缓冲区，存放这些寄存器的值。特别地，考虑到兼容性，应该预期每个节点实例都需要这样的额外缓冲区。为此，统一为所有节点准备它的寄存器缓冲。
> 这就应该是前面`struct node_info`中的内容，类似于
> ```c
> struct node_info {
>     uint64_t asid;      // AS实例标识
>     uint64_t pt_base;   // 页表基址物理地址
>     uint64_t nd_base;   // 节点入口虚拟地址（在所属AS地址空间中）
>     struct isa_reg_file regs; // 寄存器缓冲
>     // ... 其他所需信息
> }
> ```
> 更多节点自身的保存信息即存储于节点内。在页表中查询nd_base时应该预期可能不在内存中，需要时应执行VP迁移。但页表本身、至少页表的某些元数据自身是始终应该可以直接通过物理地址访问的。
> 由此，各AS实例的页表也属于硅中介层的内存，由调度器直接以物理地址访问。
>
> ---
>
> 接下来，考虑这些数据的可靠性。
> 特别地，考虑调度器日志化。调度器的每次执行都进行日志记录，并以日志作为可靠性的保障。内核性能统计也基于日志提供。日志可以选择直写内存、直写磁盘或直写远程，这里应该设置接口允许配置和重载。
> 在此之上，利用调度器的Per-Core结构与基于消息的协作方式，允许特定核心重启，清空所有运行状态并重新初始化，然后根据日志重建运行时状态恢复运行。最终做到“热重启”，在减少提供的性能但业务不中断的情况下，每次重启一个核心、累次对整个服务器完成重启。
>
> ---
>
> 最后，考虑性能优化。从调度器的逻辑可以看到，对于这些数据的访问通常不会短时间内重复读写同一个地址，但是访问的总频率较高。这也提示了Native-ISA的设计：可以提供专门的缓存控制指令，允许部分关键数据（如就绪链表与等待图等）始终保持在L2缓存中，但是不进入L1缓存，避免刚进入L1缓存就完成了调度逻辑、L1缓存再次被目标节点实例占据，形成无用缓存。
