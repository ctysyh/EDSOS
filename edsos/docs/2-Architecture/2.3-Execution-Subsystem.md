...
The EDSOS scheduler 并非传统内核代码实体，而是 comprises two orthogonal components:

- **动态指令域**:
  以Inline Stubs的形式存在，加载/节点创建时将指令直接内联至节点指令段，节点执行流自然触发。通过可信验证机制保护其完整性。这些指令声明为高特权，但其调用前后文是可信的内联指令。
  ...
  调度逻辑"无处不在却又无处集中"，恰如硅中介层之名。这个设计消除传统内核的单点瓶颈与攻击面，特别是避免了运行时从用户代码动态跳转到内核代码的脆弱过程。

- **静态内存域**:
  以Per-Core Local Data Structures的形式存在，在系统引导阶段分配。内核使用物理地址直访，完全脱离AS虚拟地址空间映射，确保崩溃隔离。
...
- **Node Activation and Context Switching**: Performing the `active` operation—reconstructing the ancestor chain, pre-warming the TLB, 围绕保存了节点执行所需完整状态的`node_info`切换上下文, and jumping to the node’s entry point.
...
作为实现，每个`node_info`包含执行所需完整状态：
```c
struct node_info {
    uint64_t asid;            // AS实例标识
    uint64_t pt_base_pa;      // 页表基址（**物理地址**，硅中介层直访）
    uint64_t entry_va;        // 节点入口虚拟地址（在所属AS地址空间中）
    struct isa_reg_file regs; // 寄存器完整状态缓冲区
    // ... 其他所需信息
};
```
考虑到兼容性，统一为所有节点准备它的寄存器缓冲，无需节点手动管理。`pt_base_pa`为物理地址，使调度器可在任何AS上下文崩溃时仍能访问页表元数据，实现资源安全回收。

值得注意的是，在页表中查询nd_base时应该预期可能不在内存中，需要时应执行VP迁移。但页表本身、至少页表的某些元数据自身是始终应该可以直接通过物理地址访问的，以至少能够重新将页表（至少是目标节点所需部分）恢复到内存/缓存中。
...
引导组件为每CPU核心分配连续物理内存区域，结构如下：
```c
// 就绪链表：数组实现的链表（消除指针解引用）
struct ready_list_entry {
    uint32_t prev_idx;  // 前驱索引（非指针）
    uint32_t next_idx;  // 后继索引
    struct node_info node; // 节点完整元数据
};

struct ready_list {
    uint32_t head_idx; // 头节点索引
    uint32_t tail_idx; // 尾节点索引
    ready_list_entry ready_list[MAX_ENTRIES]; // 连续数组
}
```
设计为双向链表有利于调度逻辑快速调整节点排序。
...
struct wait_graph_cache {
    semaphore_entry semaphores[MAX_SEMA];      // 信号量元数据行
    blocked_list_segment segments[MAX_SEG];    // 阻塞节点分段列
};

struct semaphore_entry {
    struct semaphore_token token; // 信号量的内部标识
    char semaphore_name[32];      // 信号量的代码识别名
    uint32_t trigger_msg_count;   // 触发消息数量（即本信号量触发次数）
    uint32_t trigger_msg_idx;     // 通过消息传递的首个触发消息索引
    uint32_t blocked_count;       // 阻塞节点总量
    uint32_t first_seg_idx;       // 阻塞节点列表首段索引
    uint32_t last_seg_idx;        // 尾段索引（用于追加）
}
...
```
行列二维解耦的逻辑结构，信号量条目数与每信号量阻塞节点数独立扩缩，避免全局重哈希。分段链表`blocked_list_segment`通过`next_seg_idx`链接，支持大等待队列、提高空间利用率并加速每条目节点数的扩缩容。

其中，触发消息作为列表的同时，也记录信号量的触发次数。
```c
...
struct trigger_message {
    uint32_t next_msg;    // 下一个消息的索引
    bool is_used;         // 指示本槽位是否被使用
    bool is_msg;          // 指示本槽位是否确实附带一个消息
    bool has_next;        // 是否有下一个消息
    char reserve[29];     // 保留并对齐
    uint64_t bus_message; // 调度器间的环形消息总线中消息包的物理地址
}
```
这里使用指向消息总线缓冲区的消息包指针，以避免payload复制开销。此外，同样可以进一步添加一些辅助的字段以快速定位可用的`trigger_message`。
