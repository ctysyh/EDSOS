...
> **PP size flexibility**: Because the PP is decoupled from application semantics and serves only as a physical container, it does not enforce a specific page size. While examples in this document often use 4 KiB for illustration, using larger sizes such as 2 MiB or 1 GiB is fully supported. Larger PP sizes can also help abstract away fine-grained differences across heterogeneous storage devices. From one perspective, the PP size corresponds to the granularity of a "section lock"—it only affects spatial utilization efficiency and contention on the `state` attribute in the PP NSME.
...
To organize PP NSMEs efficiently and eliminate pointer overhead, physical memory is partitioned into **physical segments**.
...
- PP size: **4 KiB**, aligned with CPU page size.
...
- **PP size**: **16 KiB**, matching typical SSD write granularities.
...
- **Hard PPSUC boundary**: Each physical machine (PM) enforces a hard limit on physical capacity—**Physical Paged Storage Unit Capacity (PPSUC)**. PPSUC is used as a **hard constraint for virtual allocation**, making the *container* relationship a mathematically provable surjection: each VP must have a PP to carry it, and the total amount is controllable. **Cross-PM PP borrowing is prohibited** to ensure certainty and locality.
...
- **PP (Physical Page)** is the managed unit of physical storage (a continuous segment of DDR or SSD);
...
- The EDSOS Native ISA must employ a sliding-window like logic when mapping VPs: the mapping result should be interpreted as **a certain physical address range of a certain size**, which may be smaller than a single PP or span multiple PPs.
