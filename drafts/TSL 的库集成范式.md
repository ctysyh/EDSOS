...
| 组件 | 角色 | TSL 表达 |
|------|------|--------|
| **库类型（Library Type）** | 封装运行时状态（上下文、缓存、资源句柄等） | `.tsltype MyLibCtx { ... }` |
| **库函数集合（`lib`）** | 提供改变结构/触发事件/复杂逻辑的操作 | `lib mylib { fn init(); fn render(); ... }` |
| **托管节点（Lib Managed Type Node, LMTN）** | 实例化库类型 + 挂载库函数 + 管理生命周期 | `node { data { (MyLibCtx) ctx; } code { instruct { cycl(ctx.exit); fnsh; } } }` |

### 1.2 调用方视角

当我们的代码的某个节点需要调用lib，我们就在代码里`psh`并`lft`一个MTN，它其中的字段正是库类型的，其中的代码正是前面提到的`lib`；而它的主逻辑`instruct`里边是一个`cycl`等待在自己专门的“退出”字段，这个字段在我们的代码认为这个节点不再需要的时候就可以通过`data`->`ance`解析赋值为`true`，这样它就执行下一条TSL指令即`fnsh`。
我们区分两种操作：一种是改变现有库字段的值、或者是从中获取值存到我们自己节点的字段，这是应该集成在库类型的运算符里的；另一种是改变现有库字段的结构、或者需要扩容/减容，这是应该包装在 lib 的 `fn` 里的。
这样，对于运算符，它不存在节点的切换，而且有很好的形参和返回槽基础，执行效率比传统的库调用高；对于 lib 的 `fn`，它在当前节点的上下文和权限里执行，但是内部自定义地解析自有库类型、或者自定义地触发自有 Event：从而解决了性能问题，并且既保持 lib 的封装性、又维护所有节点的边界。
...
- **执行隔离**：`lib` 中的 `fn` 在 **MTN 节点的上下文** 中执行，其指令只能访问：
  - 自身 `data` 字段（即 `ctx`）；
  - 通过 `ance` 声明的祖先字段（如调用方传入的 `scene`）；
- **无越权访问**：分离逻辑依然成立——MTN 拥有 `ctx`，调用方拥有 `scene`/`output`，共享通过绑定显式声明。
...
| **复杂逻辑/状态变更** | lib `fn`（在 MTN 中执行） | 一次节点切换（EDSOS 可优化） | 低频、有状态、需扩容 |
...
**TSL 的 `lib` 可直接对应传统动态库**，只需附加元信息：

| 传统 DLL | TSL 对应 |
|--------|--------|
| 导出函数（`__declspec(dllexport)`） | `lib { fn ... }` |
| 全局状态（如 TLS、static var） | 封装进 `.tsltype`，由 MTN 实例化 |
| 初始化/清理函数 | `fn init()` / `fn deinit()`，由 MTN 的 `instruct` 调用 |
| ABI 兼容性 | 通过 `.tsltype`（TSL Type）文件描述类型布局 |

> **这使 TSL 成为传统生态的“结构化封装层”**，而非替代品。
...
| 挑战 | 解决方案 |
|------|--------|
| **跨平台类型布局** | `.tsltype` 文件包含目标平台的字段偏移、对齐信息 |
| **异常/错误处理** | 返回槽中包含 `err: ErrorCode` |
| **多线程访问** | MTN 默认单线程（TS 节点天然串行）；若需并发，创建多个 MTN 实例 |
| **调试符号** | `.tsltype` + DWARF 映射，支持源码级调试 |
