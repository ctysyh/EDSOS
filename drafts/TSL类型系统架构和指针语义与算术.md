...
| 组成 | 内容 | 作用 |
|------|------|------|
| **类型名称（Type ID）** | 作用域内唯一标识符，如 `@T_i32` | 用于字段声明、跨节点一致性校验 |
...
> **本节内容是对于指针类别的类型定义要求。**
> 各类具体类型的指针本身也作为一种类型，通过 `.tsltype` 引入并在其中声明其关联性。
...
V_n = [0, L_n)
$$
所有字段（包括 `ance`）在 $V_n$ 中拥有**确定的虚拟偏移**，如同一个 struct。
