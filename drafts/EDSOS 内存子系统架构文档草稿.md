...
### 2.1 PP 的本体论
- **物理实体**：存储设备上的固定块（DRAM/SSD）
- **无生命周期、无语义、无归属**
- **状态**：仅 “盛装了 VP” 或 “空闲”
- **比喻**：井（PP）盛装水（VP），但井不主动管理水
...
| 字段 | 位宽 | 说明 |
|------|------|--------|------|
| `state` | 6 | `FREE=0`, `LOCKED=1`, `CORRUPTED=2`, `RELIABLE=3`, `RESERVED=4`,… |
| `next_free_log2` | 6 | 核心：跳跃步长提示 |
| `jump_hint_prev` | 19 | 分层链表前驱指针差值，用于加速 `next_free_log2` 的更新 |
| `jump_hint_next` | 19 | 分层链表后继指针差值，用于加速 `next_free_log2` 的更新 |
| `ded` | 1 | 奇偶校验码，供硬件使用 |
| `zone_token` | 4 | 分区标识，用于优化访问模式 |
| `reserved` | 9 | 未来扩展（如安全标签、压缩状态、ECC 标识） |

### 2.3 物理内存管理
- **分配/释放**：基于 NSME 的保守跳跃搜索（O(log d)）
- **PPSUC 硬边界**：每个 PM 的物理容量上限，不可跨 PM 借用
- **无全局回收器**：PP 释放仅由调度器主动触发
...
- **LVA** 是 TS（Tree-Stacked 进程）内部的 64 位虚拟地址，用于 CPU 执行和指针操作，始终与具体 PM 绑定；
...
\]  
这一约束确保了应用程序在进行指针算术时，可安全地将本地 LVA 转换为 GVA 用于 DPU 通信，而不会破坏程序语义。GVA 与 LVA 的生命周期和分配策略完全解耦——LVA 在 TS 创建时分配并保持稳定，而 GVA 按需延迟分配。
...
局部虚拟地址（Local Virtual Address, LVA）是 EDSOS 中 **CPU 执行视角下的唯一地址空间**。它采用标准的 64 位虚拟地址格式，由本地 MMU 通过多级页表（PTE）翻译为局部物理地址（LPA），完全兼容 x86-64 或 ARM64 的地址转换机制。LVA 的设计目标是在保留经典 OS 编程模型的同时，支撑 TS（Tree-Stacked 进程）的节点语义、作用域隔离与多核并行执行。

#### 3.2.1 LVA 的基本语义与约束

- **TS 私有性**：每个 TS 拥有独立的 LVA 空间，由 CR3 寄存器指向其根页表。不同 TS 的 LVA 空间彼此无关——即使数值相同，也无任何语义关联。切换 TS 即切换 CR3。
- **节点连续性**：一个 Node（节点）内的所有 VP（元数据 R / 指令 RX / 数据 RW）必须分配在**连续的 LVA 区域**中。这是为了：
  - 支持编译器生成的相对跳转与数据偏移；
  - 保证指针在 TS 内部的可传递性；
  - 便于子树作用域的页表隔离（通过 PTE 的 Present 位控制）。
- **稳定性要求**：LVA 在 TS 生命周期内**不可回收、不可重用**。应用指针长期有效，避免悬空指针。
...
LVA 的分配由调度器在 TS 创建和 Node 操作时触发。分配器需满足：
- 对齐节点粒度（通常 ≥ 4KB）；
- 保证节点内三段连续；
...
3. **多核安全**：`lock` 字段保护 NSME 修改，因同一 TS 可能被调度到多个 CPU Core 并行执行。
...
- `CTRN_REF`：该 VP 是 CTRN 的一部分，允许多 TS 共享写入。
...
关键特性：
- **身份唯一性**：一个 VP 在其所属 TS 的上下文中由 `{TSID, LVA}` 唯一标识；若具备跨 PM 可见性，则额外拥有一个 GVA。
- **无内建引用计数**：VP 本身**不存在传统意义上的引用计数**。它的存在性是二元的：
  - **不存在/未分配**；
  - **存在且唯一**（已分配并归属于某个 Node）。
- **绑定于单一 TS**：每个 VP 严格隶属于一个 TS，不可跨 TS 共享——除非通过 **CTRN（Cross-TS Referring Node）** 机制显式创建副本。
...
- **LVA 视图**：将 VP 嵌入 TS 的私有虚拟地址空间，形成连续节点布局；
...
- **独立身份**：每个副本拥有**独立的 LVA（在目标 TS 中）和 GVA（RefIdx > 0）**；
- **数据一致性**：由双协议保障：
  - **路径缓存协议**：最终一致性，支持乐观读写；
  - **所有权协议**：强一致性，用于锁或迁移场景。
...
#### 3.3.5 VP 与 TS/Node 的结构关系

VP 是构建 TS 地址空间的**原子砖块**，其语义由更高层结构赋予：

| 结构 | 与 VP 的关系 |
|------|-------------|
| **TS Node（节点）** | VP 的组织单元；一个 Node 包含若干**连续 LVA 的 VP**（R/RX/RW 三段） |
| **CTRN（Cross-TS Referring Node）** | 唯一允许跨 TS 共享的 Node 类型；其 VP 可被其他 TS `ref` 创建副本 |

- **VP 无结构语义**：单个 VP 不知道自身属于哪个 Node，也不知是否为 CTRN；
...
### 4.3 共享机制
#### 4.3.1 CTRN（Cross TS Referring Node）
- **唯一可跨 TS 共享的节点类型**
- **引用计数**：由 CTRN 元数据维护
- **一致性**：
...
- **缺页处理**：
  ...
  - 更新写入 TS 的 VP → 新 PP
- **CTRN 特例**：
  - fork 时 **引用计数 +1**，TS B 正常共享 CTRN
  - 写入时 **取消 CoW，允许直接写**（因 TS B 合法持有引用）
  - 若 TS B 不需要，应显式取消引用
...
- **fork**：复制 LVA 布局，共享 VP（含 CTRN 引用 +1）
...
### 7.2 后备存储
- **SSD PP**：PPSUC 包含高速磁盘，可作为 swap 目标
- **流程**：调度器写入 SSD PP → 标记 VP 为 swapped → 缺页时重载
