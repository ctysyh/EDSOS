...
> TODO List
> 1. 展开分析利用SDSC机制将上层逻辑拆分解耦、模块化，包括不限于负载均衡等。
> 2. 在调度器等待图中，进一步添加一些辅助的字段以快速定位可用的`blocked_list_segment`。
> 3. 可视化辅助：
>   - 为"双重存在形态"添加架构图
>   - 为数组链表/分段等待图添加内存布局示意图
>   - 为热重启流程添加状态转换图
> 4. 调度器日志直写远程的提案启发了内存-执行模型中对AS实例请求虚拟空间时空间尺寸限制的一个补充。由于直写远程，实际所需的有且仅有远程PM中的AS实例的虚拟空间，因此应当在远程PM的AS实例中请求好虚拟空间，然后允许本地数据源将其作为可用空间。由于跨越多PM，因此其需要特别设计提前分配逻辑，以避免本地数据源需阻塞但缺乏本地缓冲区。这个实现需要多模块协作完成
...
- [2.3 Execution Subsystem](#23-execution-subsystem)
  ...
  - [2.3.1 System Boot and Initialization](#231-system-boot-and-initialization)
    - [2.3.1.1 Building the Meta-AS from Local to Global](#2311-building-the-meta-as-from-local-to-global)
    - [2.3.1.2 Boot Constraints and Address Space Design](#2312-boot-constraints-and-address-space-design)
      - [2.3.1.2.1 Phase 1: EDSOS Early Local View](#23121-phase-1-edsos-early-local-view)
      - [2.3.1.2.2 Phase 2: Global Meta-AS Construction](#23122-phase-2-global-meta-as-construction)
    - [2.3.1.3 Additional Notes](#2313-additional-notes)
  - [2.3.2 Node Scheduler Architecture](#232-node-scheduler-architecture)
    ...
    - [2.3.2.2 Scheduling Trigger Mechanisms](#2322-scheduling-trigger-mechanisms)
      ...
      - [2.3.2.2.3 CMBS Coordination Requirement](#23223-cmbs-coordination-requirement)
    ...
    - [2.3.2.4 Chained Message Bus for Scheduler (CMBS)](#2324-chained-message-bus-for-scheduler-cmbs)
      ...
      - [2.3.2.5 调度器日志化](#2325-调度器日志化)
        - [日志驱动的热重启机制](#日志驱动的热重启机制)
  - [2.3.3 Synchronization Mechanisms](#233-synchronization-mechanisms)
    ...
    - [2.3.3.2 Scheduler-Driven Semaphore Closure (SDSC)](#2332-scheduler-driven-semaphore-closure-sdsc)
      ...
      - [2.3.3.2.4 SDSC: Decoupling Scheduling Policies from Hot Path](#23324-sdsc-decoupling-scheduling-policies-from-hot-path)
  - [2.3.4 Load Balancing](#234-load-balancing)
    ...
    - [2.3.4.2 Decentralized Negotiation: Request–Response over the CMBS](#2342-decentralized-negotiation-requestresponse-over-the-cmbs)
...
- **Decentralized, Locally Self-Organizing**: Load balancing, synchronization, and coordination emerge from peer-to-peer interactions via the Chained Message Bus for Scheduler (CMBS), no global coordinator.
...
## 2.3.1 System Boot and Initialization

### 2.3.1.1 Building the Meta-AS from Local to Global
...
### 2.3.1.2 Boot Constraints and Address Space Design
...
The memory subsystem (see [§2.1](./2.1-Memory-Subsystem.md)) provides a well-formed LVA space for execution, while GVA resolution is delegated to DPU-managed infrastructure.

This design allows EDSOS to present a **globally coherent abstraction** without requiring hardware modifications.

#### 2.3.1.2.1 Phase 1: EDSOS Early Local View
...
#### 2.3.1.2.2 Phase 2: Global Meta-AS Construction
...
The EDSOS scheduler is not a conventional kernel code entity; instead, it comprises two orthogonal components:

- **Dynamic Instruction Domain**:
  Manifested as Inline Stubs, these instructions are directly inlined into the node's instruction segment during loading or node creating, naturally triggered by the node's execution flow. Their integrity is protected through a trusted verification mechanism. These instructions are declared as high-privilege, yet their calling context consists of trusted inline sequences ensured its safety.
  ...
  The scheduling logic is "ubiquitous yet nowhere centralized", aptly echoing the namesake of the Silicon Interposer. This design eliminates the single-point bottleneck and attack surface inherent in traditional kernels, particularly avoiding the fragile runtime transition from user code to kernel code.

- **Static Memory Domain**:
  Expressed as Per-Core Local Data Structures, this memory domain is allocated during the system boot phase. The kernel accesses these using physical addresses directly, completely decoupled from AS virtual address space mappings, ensuring crash isolation.
  ...
  - A **Wait Graph**: a mapping from semaphore identifiers to lists of blocked nodes;
  - **Cache Space for Chained Message Bus**: see [§2.3.2.4](#2324-chained-message-bus-for-scheduler) for details;
...
- Cross-core coordination is achieved asynchronously via the CMBS, without blocking the local scheduling path.
...
- **Signal-Driven Synchronization**: Managing the wait graph to implement O(1) `wait(semaphore)` and `signal(semaphore)` semantics.
- **Node Activation and Context Switching**: Performing the `active` operation—reconstructing the ancestor chain, pre-warming the TLB, switching between contexts according to `node_info` which reserves the complete state information for nodes executing, and jumping to the node’s entry point.
...
- **CMBS Endpoint**: Acting as a CMBS endpoint to trigger and process cross-core signals (e.g., migration requests, remote wakeups).
...
As an implementation detail, each `node_info` contains the complete state required for execution:

```c
struct node_info {
    uint64_t asid;            // AS instance identifier
    uint64_t pt_base_pa;      // Page table base address (**physical address**, directly accessed by the Silicon Interposer)
    uint64_t entry_va;        // Node entry virtual address (within the owning AS address space)
    struct isa_reg_file regs; // Complete register state buffer
    // ... additional fields as required
};
```

For compatibility considerations, register buffers are uniformly prepared for all nodes, eliminating the need for manual management by individual nodes. The `pt_base_pa` field is specified as a physical address, enabling the scheduler to access page table metadata even when any AS context has crashed, thereby ensuring safe resource reclamation.

It is noteworthy that when querying `nd_base` in the page table, one should anticipate the possibility that it may not reside in memory; in such cases, VP (Virtual Page) migration should be performed as needed. However, the page table itself, or at least certain metadata thereof, must always remain directly accessible via physical addresses, sufficient to restore the page table (or at least the portions required by the target node) back into memory or cache.

---

### 2.3.2.2 Scheduling Trigger Mechanisms

EDSOS scheduling is triggered through two orthogonal mechanisms: **cooperative scheduling** and **preemptive scheduling**. Both are realized via instruction sequences statically inlined by the EDSOS Loader during the loading phase. Although they differ in timing and context, they share the same underlying scheduling logic. Critically, **all scheduling paths integrate processing of the CMBS** to preserve system-wide coordination under decentralization. Together, these interfaces constitute the scheduler’s **Normal Open Interface (NOI)**.
...
- **Entry Stub Functions**:
  ...
  - Invoke `check()` to process inbound messages from the CMBS.
- **Exit Stub Functions**:
...
- **Behavior**: In addition to performing the requested operation, these stubs invoke `check()` to process the CMBS.
- **Interface Classification**: These stubs are part of the scheduler’s NOI and are directly invocable by user code (see [§2.3.4.1](#2341-load-balancing-policy-and-trigger-conditions)).
...
#### 2.3.2.2.3 CMBS Coordination Requirement

All scheduling trigger paths—cooperative or preemptive—**mandatorily invoke `check()`**. This requirement arises because:

- The CMBS employs a decentralized SPSC (single-producer, single-consumer) ring buffer per core pair;
...
The EDSOS scheduler employs a **Distributed Ready Forest** as its core ready-state management abstraction. This structure is not a monolithic global data structure; rather, it is the logical union of per-core **Ready Linked Lists**, each maintained independently by a CPU core. These local lists coordinate via the **Chained Message Bus** (see [§2.3.2.4](#2324-chained-message-bus-for-scheduler)) to enable flexible scheduling policies and structural affinity optimizations, collectively forming a decentralized, highly scalable task dispatching mechanism.
...
3. **Implicit Home-Core Affinity**  
   Newly created AS nodes are instantiated via a `push` operation by the current core’s scheduler and are automatically enqueued into that core’s Ready List. Consequently, the entire subtree executes by default on its home core, ensuring strong locality. Node migration is an explicit operation triggered only by load-balancing mechanisms (see [§2.3.4](#234-load-balancing)).

During system boot, the boot loader allocates a contiguous physical memory region for each CPU core with the following structure:

```c
// Ready list: array-based linked list (eliminates pointer dereferencing)
struct ready_list_entry {
    uint32_t prev_idx;      // Previous entry index
    uint32_t next_idx;      // Next entry index
    struct node_info node;  // Complete node metadata
};

struct ready_list {
    uint32_t head_idx; // Head entry index
    uint32_t tail_idx; // Tail entry index
    ready_list_entry ready_list[MAX_ENTRIES]; // Contiguous array
}
```

The doubly-linked list design facilitates rapid adjustment of node ordering by the scheduling logic.

#### 2.3.2.3.2 Per-Core Wait Graph

The Wait Graph implements the `wait(semaphore)` and `signal(semaphore)` synchronization primitives in EDSOS. Designed for performance, memory efficiency, and namespace isolation, it consists of three per-core components:
...
2. **Local Semaphore Name Hash Map**  
   - Type: Hash map mapping `local_semaphore_name → local_semaphore_id`.
   - `local_semaphore_name` is scoped to the current AS instance (e.g., `"mutex_lock_0x1234"`);
   - `local_semaphore_id` is drawn from a per-core ID space (e.g., `[0, N₀)` );
...
3. **Global Semaphore Name Hash Map**  
   - Type: Hash map mapping `global_semaphore_name → local_semaphore_id`.
   - `global_semaphore_name` is globally unique (e.g., `"io_complete_disk0_blk123"`);
   - `local_semaphore_id` is assigned from a reserved high-ID range (e.g., ≥ 2³¹), with the MSB set to distinguish global IDs;
...
The memory region is structured as follows:
```c
// Wait graph: segmented two-dimensional structure
struct wait_graph_cache {
    semaphore_entry semaphores[MAX_SEMA];      // Rows of semaphore metadata entries
    blocked_list_segment segments[MAX_SEG];    // Columns of blocked-node segments
};

struct semaphore_entry {
    struct semaphore_token token; // Internal identifier for the semaphore
    char semaphore_name[32];      // Human-readable code identifier for the semaphore
    uint32_t trigger_msg_count;   // Number of trigger messages (as signaling times)
    uint32_t trigger_msg_idx;     // Index of the first trigger message delivered via message passing
    uint32_t blocked_count;       // Total number of blocked nodes
    uint32_t first_seg_idx;       // Index of the first segment in the blocked-node list
    uint32_t last_seg_idx;        // Index of the last segment (for appending)
}

struct blocked_list_segment {
    struct semaphore_token token;
    struct node_info blocked_node[MAX_SEGMENT_SIZE];
    uint32_t next_seg_idx;
}

struct semaphore_token {
    uint64_t id        : 63;
    uint64_t is_global : 1;   // 0 indicates a local semaphore, 1 indicates a registered global semaphore.
}
```

This row-column, two-dimensional decoupled structure allows the number of semaphore entries and the number of blocked nodes per semaphore to scale independently, avoiding global rehashing. The segmented list `blocked_list_segment` is linked via `next_seg_idx`, supporting large wait queues, improving space utilization, and accelerating expansion and contraction of nodes per entry.
Meanwhile, trigger messages serve as a list while also recording the signal count for the semaphore.

```c
struct trigger_message_cache {
    struct trigger_message msg[MAX_MESSAGE_COUNT];
}

struct trigger_message {
    uint32_t next_msg;      // Index of the next message
    bool is_used;           // Indicates whether this slot is in use
    bool is_msg;            // Indicates whether this slot actually carries a message
    bool has_next;          // Indicates whether another message follows
    char reserve[29];       // Reserved for alignment
    uint64_t cmbs_message;  // Physical address of the message packet in the Chained Message Bus for Scheduler buffer
}
```

Here, a pointer to the message packet in the message bus buffer is used to avoid payload copying overhead. Additionally, auxiliary fields may be introduced to facilitate rapid location of available `trigger_message` slots.

---

### 2.3.2.4 Chained Message Bus for Scheduler (CMBS)
...
Logically, the CMBS forms a directed graph. Each node maintains a set of **Single-Producer Single-Consumer (SPSC) lock-free queues**—one per direct neighbor—as inbound message buffers. The node acts as the consumer; its immediate predecessor(s) in the topology act as producer(s).
...
As a critical EDSOS component, the CMBS is initialized during system boot and torn down at shutdown.

---

#### 2.3.2.5 调度器日志化

- 调度器的每次操作都进行日志记录，并以日志作为可靠性的保障
- 内核性能统计也基于日志提供
- 多后端支持：内存环形缓冲（低延迟）、NVMe（持久化）、远程DPU（集群恢复）；正常情况都不会预期短期内需要读取因此避免缓存。这里应该设置接口允许配置和重载，并允许同时采取多个输出目标以提高可用性。

##### 日志驱动的热重启机制

利用调度器的Per-Core结构与基于消息的松耦合协作方式，允许特定核心重启，清空所有运行状态并重新初始化，然后根据日志重建运行时状态恢复运行。最终做到滚动重启，在减少提供的性能（可用核心数）但业务不中断的情况下，每次重启一个核心及其本地硅中介层、累次对整个服务器完成重启。
- **热重启流程**：
  1. 核心故障检测 → 清空本地状态
  2. 重放日志重建：节点树、等待图、寄存器快照
  3. 无缝接管执行，业务无感知中断
- **运维价值**：支持集群核心滚动重启，实现"零停机维护"
...
- **No physical representation**: The semaphore does not occupy memory, correspond to any in-kernel or user-space data structure, or expose a handle. It is identified solely by a globally unique name (`semaphore_name`).
...
- **Acts as an execution barrier**: When a node invokes `wait(semaphore_name)`, it voluntarily yields the CPU and registers itself in the associated wait queue. Its execution flow halts until another execution context calls `signal(semaphore_name)`, at which point the scheduler transitions the node to the `ready` state.
...
- **Core data structure**: Each per-core scheduler maintains a local **Wait Graph** (see [§2.3.2.3.2](#23232-per-core-wait-graph)).
- **`wait` operation flow**:
  1. The node executes `edsos_wait(semaphore_name)`.
  2. The inline scheduler stub transitions the node’s state from `running` to `blocked`.
  3. The scheduler locates or creates a slot for `semaphore_name` in its local Wait Graph and enqueues the node into the associated FIFO queue.
...
- **`signal` operation flow**:
  1. The node executes `edsos_signal(semaphore_name)`.
  2. The inline stub queries the local Wait Graph for the queue associated with `semaphore_name`.
...
1. **QUERY**:  
   The signaling core (the *Giver Core*) broadcasts a `QUERY(semaphore_name)` message via the **Chained Message Bus**.

2. **RESPONSE**:  
   Each receiving core inspects its local Wait Graph. If it hosts waiters for `semaphore_name`, it replies with a `RESPONSE` containing:
   - Its core ID,
   - A timestamp (typically derived from `RDTSC`) to approximate cross-core FIFO fairness,
   - Metadata such as queue length.

3. **GRANT**:  
   The Giver Core aggregates all `RESPONSE` messages and selects a *Winner Core* based on a predefined policy (e.g., earliest timestamp). It then sends a `GRANT(semaphore_name)` message exclusively to that core.
...
- **Deep scheduler integration**: The negotiation proceeds asynchronously over the Chained Message Bus and is processed periodically by the scheduler’s `check()` routine (see [§2.3.2.4](#2324-chained-message-bus-for-scheduler)), without blocking normal execution.
...
> **Note**: Only AS nodes containing a valid executable segment may register SDSCs. The format and semantics of signal payloads are defined in [§2.3.2.4.2](#23242-message-format).

#### 2.3.3.2.4 SDSC: Decoupling Scheduling Policies from Hot Path

SDSC机制使调度策略从热路径中彻底解耦，实现**可插拔、可验证、可热更新**的策略模块化：

| 应用场景 | SDSC触发信号量 | 执行逻辑 | 优势 |
|----------|----------------|----------|------|
| **就绪链表重排序** | `rebalance_trigger` | 基于负载/亲和性重排`ready_list`索引 | 热路径实现极小常数O(1)，仅需时触发 |
| **负载均衡决策** | `steal_request` | 评估子树可迁移性，生成`MIGRATE_SUBTREE`消息 | 策略与迁移协议解耦 |
| **NUMA感知调度** | `numa_hint` | 调整节点`home_core`字段 | 无需修改调度器核心代码 |

EDSOS仅需部署新SDSC即可实现新调度策略。策略代码受SDSC校验规则限制，在严格边界内安全执行。
...
EDSOS employs a **Work-Stealing++** mechanism for load balancing. As an extension of the classic work-stealing model, it reorients around structural semantics rather than numeric workload equality. The primary objective is not to equalize the number of ready nodes across CPU cores, but to **maximize structural affinity**, **minimize context-switch overhead**, and **reduce inter-core communication costs**.

Work-Stealing++ is fully decentralized and operates at the granularity of **Arbor Strux (AS) subtrees**. Coordination between cores is mediated via the CMBS, with **proxy nodes** used to cache remote structural metadata and amortize coordination latency.
...
- **Anomalous Load Spikes**: An overload alert is received via the CMBS or an inter-processor interrupt (IPI). For example, when a neighboring core’s ready queue exceeds its high-water mark.
...
本节描述的决策与执行逻辑均通过SDSC实现，详见[§2.3.3.2.4](#23324-sdsc-decoupling-scheduling-policies-from-hot-path)。

---

### 2.3.4.2 Decentralized Negotiation: Request–Response over the CMBS
...
All messages are transmitted asynchronously over the CMBS and do not block the local scheduler.
...
3. **Migration Initiation**  
   The Giver sends a `MIGRATE_SUBTREE(subtree_root_gva, target_core)` command via the CMBS to initiate the transfer.
...
2. **Metadata Transfer**:  
   The Giver transmits the subtree root’s Local Virtual Address (LVA) and topology descriptor to the Taker via the CMBS.
...
- Receiving downstream structural updates (e.g., `detach` or `finish` semaphores).
